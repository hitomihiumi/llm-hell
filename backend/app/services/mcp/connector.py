...
class SearchContext:
    ...
    # concurrently, and they land on the same documents: reading one
    # spreadsheet's three tabs once per phrasing cost twelve round trips
    # inside a twenty-second budget. A connector's own attributes are the
    # wrong place for that - the registry keeps one instance for the life of
    # the process, so anything cached there outlives the edit that
    # invalidates it.
